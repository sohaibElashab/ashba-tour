...
const FLEET_EXCEL = path.join(MEDIA_DIR, 'Tableau de Gestion de Flotte Touristique.xlsx');
...
function parseFleet() {
    ...
    const fleet = rawData.map(row => {
        ...
        const id = idRaw ? String(idRaw).trim() : `V${Math.floor(Math.random() * 1000)}`;
        
        const model = row['Marque & Modèle'] || row['Marque'] || row['Model'] || row[keys[1]] || 'Unknown Model';
        const places = row['Places'] || row['Capacity'] || row[keys[2]] || 0;
        const status = row['État (Statut)'] || row['Etat'] || row['Status'] || row[keys[3]] || 'Disponible';
        const price = row['prix'] || row['Prix'] || row['Price'] || row[keys[4]] || 'Sur devis';
        const luggage = row['luggage'] || row['Luggage'] || row['Bagages'] || row[keys[5]] || '';
    ...
    fs.writeFileSync(path.join(DATA_DIR, 'fleet.json'), JSON.stringify(fleet, null, 2));
...
function parseTours() {
    ...
    const tours = rawData.map((row, index) => {
        ...
        const title = row['title'] || row['Title'] || row[keys[1]] || 'Untitled Tour';
        const description = row['description'] || row['Description'] || row[keys[2]] || '';
        const vehicleType = row['vehicleType'] || row['VehicleType'] || row[keys[3]] || '';
        ...
        const category = row['category'] || row['Category'] || row[keys[7]] || 'General';
        const featuredRaw = row['featured'] || row['Featured'] || row[keys[8]];
        const featured = String(featuredRaw).toLowerCase() === 'true' || featuredRaw === true;

        // Image Logic: Exact filename match in /public/media
        let imagePath = '/placeholder.svg';
        if (imageFilename) {
            const foundImage = mediaFiles.find(file => file.toLowerCase() === String(imageFilename).trim().toLowerCase());
            if (foundImage) {
                imagePath = `/media/${foundImage}`;
            } else {
                console.warn(`⚠️ Tour image not found: ${imageFilename} for tour ${id}`);
            }
        }
    ...
    fs.writeFileSync(path.join(DATA_DIR, 'tours.json'), JSON.stringify(tours, null, 2));
