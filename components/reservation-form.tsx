  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      reservation: {
        ...
        estimatedTotal: total ? `${total} EUR` : "On Demand",
      },
    };

    try {
      const res = await fetch("/api/email/reservation", {
        method: "POST",
            ...
            {type === "general" && (
              <div className="flex space-x-2 mt-2 md:mt-0">
                <button
                  type="button"
                  onClick={() =>
                    setFormData({ ...formData, serviceType: "transfer" })
                  }
                ...
                <button
                  type="button"
                  onClick={() =>
                    setFormData({ ...formData, serviceType: "tour" })
                  }
              ...
              */}
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                {fleet.map((vehicle) => (
                  <label
                    key={vehicle.id}
                    className={`relative cursor-pointer group h-24 sm:h-32 overflow-hidden rounded-xl ${type === "fleet" && formData.vehicle !== vehicle.id ? "hidden" : ""} ${type === "fleet" ? "col-span-full h-48" : ""}`}
                  >
                    <input
                      type="radio"
                      name="vehicle"
                    ...
                    <div
                      ...
                      style={{
                        backgroundImage: `url(${vehicle.images?.[0] || "/placeholder.svg"})`,
                      }}
                    />
                    ...
                    <div className="relative h-full flex flex-col items-center justify-center text-white p-2 text-center">
                      <div className="font-bold text-base sm:text-lg leading-tight mb-1">
                        {vehicle.model}
                      </div>
