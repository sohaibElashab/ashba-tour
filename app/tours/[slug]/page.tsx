  ...
  return (
    <main className="min-h-screen bg-background">
      ...
      <div className="relative h-[50vh] min-h-[400px]">
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="text-center text-white px-4">
            <h1 className="text-4xl md:text-6xl font-display font-bold mb-4">{tour.title}</h1>
            <div className="flex items-center justify-center gap-4 text-sm md:text-base">
               {tour.category && <span className="bg-primary px-3 py-1 rounded-full">{tour.category}</span>}
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-12">
        <Link href="/tours" className="inline-flex items-center gap-2 text-muted-foreground hover:text-primary mb-8 transition-colors">
          <ArrowLeft size={20} />
          Back to all tours
        </Link>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <section className="bg-card p-6 rounded-xl border border-border shadow-sm">
               <h2 className="text-2xl font-bold mb-6 text-foreground">Key Features & Amenities</h2>
               <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {tour.features ? tour.features.split(/\\r\\n|\r|\n/).filter((f:string) => f.trim().length > 0).map((feature:string, idx:number) => (
            ...
            <section className="bg-card p-6 rounded-xl border border-border shadow-sm">
               <h2 className="text-2xl font-bold mb-6 text-foreground">Vehicle Information</h2>
               <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                 <div className="flex items-center gap-4 p-4 bg-muted/50 rounded-lg">
                    <Users size={32} className="text-primary" />
                    <div>
                      <p className="text-sm text-muted-foreground">Capacity</p>
                      <p className="font-semibold text-lg">{tour.capacity || "N/A"}</p>
                    </div>
                 </div>
                 <div className="flex items-center gap-4 p-4 bg-muted/50 rounded-lg">
                    ...
                    <div>
                      ...
                      <p className="font-semibold text-lg">{tour.vehicleType || "Standard"}</p>
                    </div>
                 </div>
               </div>
            </section>
            ...
            <div className="sticky top-24">
              <div className="bg-card p-6 rounded-xl border border-border shadow-lg">
                ...
                <p className="text-sm text-muted-foreground mb-6">
                  Click the button below to reserve <strong>{tour.title}</strong>.
                </p>
